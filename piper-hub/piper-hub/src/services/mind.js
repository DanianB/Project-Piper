...
const DEFAULT_MIND = {
  ...
  // Long-term user taste model (reinforcement). Counts accumulate over time.
  userTaste: { likes: {}, dislikes: {}, updatedAt: 0 },
...
function loadMind() {
  ...
  try {
    if (fs.existsSync(MIND_PATH)) {
      ...
      _mind.userTaste = _mind.userTaste || { likes: {}, dislikes: {}, updatedAt: 0 };
...
export function getSessionState(sessionId = "default") {
  ...
  const s = {
    ...
    // Topic tracking for pronoun/last-topic resolution
    lastUserTopic: "",
    lastAssistantTopic: "",

    // Rolling session summary (bullets) to improve coherence
    sessionSummary: [],
    lastSummaryTurn: 0,

    // Short conversational memory for coherence (in-RAM per session). for coherence (in-RAM per session).
...
export function setLastTopics(sessionId, { userTopic = "", assistantTopic = "" } = {}) {
  const s = getSessionState(sessionId);
  if (userTopic) s.lastUserTopic = String(userTopic).slice(0, 220);
  if (assistantTopic) s.lastAssistantTopic = String(assistantTopic).slice(0, 220);
}

export function getLastTopics(sessionId) {
  const s = getSessionState(sessionId);
  return { userTopic: s.lastUserTopic || "", assistantTopic: s.lastAssistantTopic || "" };
}

export function getSessionSummary(sessionId) {
  const s = getSessionState(sessionId);
  return Array.isArray(s.sessionSummary) ? s.sessionSummary.slice() : [];
}

function simpleTopicFromText(text) {
  const t = String(text || "").trim();
  if (!t) return "";
  // strip leading greetings
  const cleaned = t.replace(/^(hello|hi|hey|good\s(morning|day|evening))[!,.\s]+/i, "").trim();
  // keep it short
  return cleaned.slice(0, 120);
}

export function updateSessionSummaryIfNeeded(sessionId, { userMsg = "", assistantMsg = "" } = {}) {
  const s = getSessionState(sessionId);
  const mind = getMind();
  const turns = s.turns || 0;
  const everyN = Number(process.env.PIPER_SUMMARY_EVERY_N_TURNS || "6");
  const minGap = Math.max(2, everyN);

  if (turns - (s.lastSummaryTurn || 0) < minGap) return;

  const bullets = [];
  const topic = s.lastUserTopic || s.lastAssistantTopic || simpleTopicFromText(userMsg) || "";
  if (topic) bullets.push(`Current thread: ${topic}`);

  // recent opinion (if any)
  const lastOpKey = s.lastOpinionKey || "";
  if (lastOpKey) {
    const op = (mind.opinions || {})[lastOpKey];
    if (op) {
      const stance = String(op.stance || "mixed");
      const score = Number(op.score ?? 0).toFixed(2);
      bullets.push(`My stance: ${lastOpKey.replace(/^topic:/, "")} is ${stance} (${score}).`);
      if (op.rationale) bullets.push(`Why: ${String(op.rationale).slice(0, 120)}`);
    }
  }

  // user taste hints
  const likes = Object.entries(mind.userTaste?.likes || {}).sort((a,b)=>b[1]-a[1]).slice(0,2);
  if (likes.length) bullets.push(`You tend to like: ${likes.map(([k])=>k.replace(/^topic:/,"")).join(", ")}.`);

  const dislikes = Object.entries(mind.userTaste?.dislikes || {}).sort((a,b)=>b[1]-a[1]).slice(0,2);
  if (dislikes.length) bullets.push(`You tend to dislike: ${dislikes.map(([k])=>k.replace(/^topic:/,"")).join(", ")}.`);

  // open last user request snippet
  if (userMsg) bullets.push(`Last thing you said: ${String(userMsg).slice(0, 140)}`);

  s.sessionSummary = bullets.slice(0, 5);
  s.lastSummaryTurn = turns;
}

export function recordUserPreference(rawTopic, signal, strength = 1) {
  const mind = getMind();
  const key = normalizeTopicKey(rawTopic);
  if (!key) return { ok: false };

  const taste = mind.userTaste || (mind.userTaste = { likes: {}, dislikes: {}, updatedAt: 0 });
  const n = Math.max(1, Math.min(5, Number(strength) || 1));

  if (signal === "like") {
    taste.likes[key] = (taste.likes[key] || 0) + n;
  } else if (signal === "dislike") {
    taste.dislikes[key] = (taste.dislikes[key] || 0) + n;
  }
  taste.updatedAt = nowMs();
  mind.updatedAt = nowMs();
  saveMind();
  return { ok: true, key, likes: taste.likes[key] || 0, dislikes: taste.dislikes[key] || 0 };
}

export function getUserAffinity(rawTopic) {
  const mind = getMind();
  const key = normalizeTopicKey(rawTopic);
  const likes = Number(mind.userTaste?.likes?.[key] || 0);
  const dislikes = Number(mind.userTaste?.dislikes?.[key] || 0);
  return { key, likes, dislikes, net: likes - dislikes };
}
