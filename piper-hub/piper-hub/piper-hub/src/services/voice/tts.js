...
function emotionToChatterboxParams(base, emotion, intensity, mood) {
  ...
  // Subtle mood modulation: positive mood slightly livens delivery, negative tightens.
  const m = mood == null ? 0 : clamp(Number(mood) || 0, -1, 1);
  const moodMod = m;
  exaggeration += 0.08 * moodMod;
  temperature += 0.05 * moodMod;
  cfg_weight -= 0.04 * moodMod;
...
async function runChatterboxToWav(text, voiceId = "default", meta = {}) {
  ...
  const intensity = meta?.intensity ?? 0.4;
    ...
    const mood = typeof meta?.mood === "number" ? meta.mood : null;
