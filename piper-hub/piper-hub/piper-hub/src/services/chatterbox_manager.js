...
async function maybePrewarm() {
  if (prewarmed) return;
  if (String(process.env.CHATTERBOX_PREWARM || "").toLowerCase() !== "1") return;
  // Only prewarm if healthy; keep it lightweight.
  const endpoint = `http://${HOST}:${PORT}/audio/speech`;
  try {
    const r = await httpPostJson(endpoint, { input: "warm up", voice: "default", max_new_tokens: 48 });
    if (r.status >= 200 && r.status < 300) {
      prewarmed = true;
      console.log("[chatterbox] prewarm ok");
    } else {
      console.log("[chatterbox] prewarm non-2xx", { status: r.status });
    }
  } catch (e) {
    console.log("[chatterbox] prewarm failed", { err: String(e?.message || e) });
  }
}
...
export async function ensureChatterboxProcess() {
  if (await isHealthy()) { await maybePrewarm(); return { ok: true, adopted: true }; }
  ...
  starting = (async () => {
    ...
    for (let i = 0; i < 40; i++) {
      if (await isHealthy()) { await maybePrewarm(); return { ok: true, adopted: false }; }
      await sleep(500);
    }
