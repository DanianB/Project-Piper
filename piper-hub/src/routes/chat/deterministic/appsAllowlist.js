// src/routes/chat/deterministic/appsAllowlist.js
import fs from "fs";
import path from "path";

export function readAppsAllowlist() {
  try {
...
export function listAllowlistedAppIds() {
  const apps = readAppsAllowlist();
  return Object.keys(apps || {}).filter(Boolean).sort();
}

export function isListAppsIntent(msg) {
  const t = String(msg || "").trim().toLowerCase();
  if (!t) return false;
  // allow polite fillers
  const s = t
    .replace(
      /\b(please|pls|sir|piper|hey|hi|hello|can you|could you|would you)\b/g,
      " "
    )
    .replace(/\s+/g, " ")
    .trim();
...
export function parseOpenAllowlistedApp(msg) {
  const t = String(msg || "").trim().toLowerCase();
