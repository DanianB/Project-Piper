...
// De-godfile refactor: keep legacy routing logic intact, but move helper blocks
// (deterministic parsing, ids, policies, constants) into small modules.
...
} from "./deterministic/appsAllowlist.js";
import {
  setLastOpenedFolder,
  getLastOpenedFolder,
  parseOpenDesktopFolder,
  parseWriteTextInThere,
} from "./deterministic/filesystem.js";
import { isGreeting } from "./deterministic/greetings.js";
import { isWebSearchIntent, extractWebQuery } from "./deterministic/webIntent.js";
...
// --- Phase 4 Plan Mode ---
try {
  const currentPlan = getCurrentPlan(sid);

  // Text commands for plan control
  if (isAbortPlanCommand(msg)) {
    ...
    pushConversationTurn(sid, "assistant", reply);
    return res.json({ reply, emotion: "neutral", intensity: 0.35, proposed: [], meta: { affect, plan, sources: EMPTY_SOURCES } });
  }

  if (isApprovePlanCommand(msg) && currentPlan && currentPlan.status === "proposed") {
    const affect = getAffectSnapshot(sid);
    const plan = setPlanStatus(sid, "approved");
    const reply = enforcePiper("Plan approved. Tell me if you'd like me to start step 1 (inspect) or gather docs, sir.");
    pushConversationTurn(sid, "assistant", reply);
    return res.json({ reply, emotion: "neutral", intensity: 0.35, proposed: [], meta: { affect, plan, sources: EMPTY_SOURCES } });
  }

  // Create a plan for high-level, multi-step requests
  if (looksLikeHighLevelProject(msg)) {
    const affect = getAffectSnapshot(sid);
    const goal = msg;
    const plan = draftGenericIntegrationPlan(sid, goal);
    console.log("[chat] plan", { sid, ms: Date.now() - started, proposed: 0, mood: affect?.mood, fr: affect?.frustration?.total || 0 });
    const reply = enforcePiper(`I drafted a plan for that, sir. Review it, then say "approve plan" or press Approve.`);
    pushConversationTurn(sid, "assistant", reply);
    return res.json({ reply, emotion: "neutral", intensity: 0.35, proposed: [], meta: { affect, plan, sources: EMPTY_SOURCES } });
  }
} catch (e) {
  console.log("[chat] plan ERROR", { sid, err: String(e?.message || e) });
}
...
export const chatRoutes = buildChatRouter;
